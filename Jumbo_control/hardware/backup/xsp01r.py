...
class XSP01R:
    ...
    def kryo1_einschalten(self):
        """Schaltet Kryo 1 komplett ein (System + Remote)."""
        bits = self._ausgaenge_lesen()
        bits |= (1 << BIT_KRYO1_SYSTEM) | (1 << BIT_KRYO1_REMOTE)
        self._ausgaenge_setzen(bits)
        print("[XSP01R] Kryo 1 EIN (System + Remote)")

    def kryo1_ausschalten(self):
        """Schaltet Kryo 1 komplett aus (System + Remote)."""
        bits = self._ausgaenge_lesen()
        bits &= ~((1 << BIT_KRYO1_SYSTEM) | (1 << BIT_KRYO1_REMOTE))
        self._ausgaenge_setzen(bits)
        print("[XSP01R] Kryo 1 AUS (System + Remote)")

    def kryo2_einschalten(self):
        """Schaltet Kryo 2 komplett ein (System + Remote)."""
        bits = self._ausgaenge_lesen()
        bits |= (1 << BIT_KRYO2_SYSTEM) | (1 << BIT_KRYO2_REMOTE)
        self._ausgaenge_setzen(bits)
        print("[XSP01R] Kryo 2 EIN (System + Remote)")

    def kryo2_ausschalten(self):
        """Schaltet Kryo 2 komplett aus (System + Remote)."""
        bits = self._ausgaenge_lesen()
        bits &= ~((1 << BIT_KRYO2_SYSTEM) | (1 << BIT_KRYO2_REMOTE))
        self._ausgaenge_setzen(bits)
        print("[XSP01R] Kryo 2 AUS (System + Remote)")
