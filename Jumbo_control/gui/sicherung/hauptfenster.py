...
class Hauptfenster(QMainWindow):
    ...
    def _build_timing_panel(self) -> QWidget:
        ...
        self._dt_regen_start.setDateTime(self._dt_regen_start.dateTime().currentDateTime())
        ...
        self._dt_regen_stop.setDateTime(self._dt_regen_stop.dateTime().currentDateTime())
        ...
        self._dt_kryo.setDateTime(self._dt_kryo.dateTime().currentDateTime())
    ...
    def _regen_toggle(self, aktiv: bool):
        if aktiv:
            ...
            start = self._dt_regen_start.dateTime().toPyDateTime()
            stop  = self._dt_regen_stop.dateTime().toPyDateTime()
            if stop <= start:
                ...
                try:
                    if name == "Kryo 1":
                        ...
                        if port:
                            ...
                            c.beenden()
                    self._kryo_panel.kryo_ein_signal.emit(name) if an else None
                except Exception as e:
    ...
    def _kryo_timer_toggle(self, aktiv: bool):
        if aktiv:
            ...
            if not ausgewaehlte:
                ...
                return
            ziel = self._dt_kryo.dateTime().toPyDateTime()
            from datetime import datetime
    ...
    def _kryo_timer_tick(self):
        ...
        if verbleibend <= 0:
            ...
            def _einschalten():
                ...
                # Signal direkt aus Thread emittieren – Qt macht das thread-sicher
                for name in ausgewaehlte:
    ...
    def _mitternacht_callback(self):
        ...
        try:
            pfad = self._pdf_erstellen(datum=gestern)
    ...
    def _pdf_erstellen(self, datum: datetime | None = None) -> str:
        """Holt Daten aus den Panels und erzeugt das PDF."""
