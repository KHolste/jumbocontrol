...
class KryoZeile(QWidget):
    ...
    def __init__(self, name: str, ist_xsp: bool = False, kryo_nr: int = 0, parent=None):
        ...
        self._lbl_wartung = QLabel("")
        self._lbl_wartung.setStyleSheet("font-size: 9px; color: #666;")
        layout.addWidget(self._lbl_wartung, 1)
    ...
    def aktualisieren(self, status: dict):
        if not status.get("gueltig"):
            ...
            self._lbl_wartung.setText("keine Verbindung")
            return
        ...
        farbe = FARBE_ROT if (faellig or fehler) else (FARBE_GELB if wartung_in <= WARTUNG_GELB_AB else FARBE_GRUEN)
        ...
        if faellig:
            self._lbl_wartung.setText("⚠ Wartung fällig!")
            self._lbl_wartung.setStyleSheet(f"font-size: 9px; color: {FARBE_ROT}; font-weight: bold;")
        elif fehler:
            self._lbl_wartung.setText(f"⚠ {fehler[0]}")
            self._lbl_wartung.setStyleSheet(f"font-size: 9px; color: {FARBE_ROT};")
        elif wartung_in <= WARTUNG_GELB_AB:
            self._lbl_wartung.setText(f"Wartung in {wartung_in} h")
            self._lbl_wartung.setStyleSheet(f"font-size: 9px; color: {FARBE_GELB};")
        else:
            self._lbl_wartung.setText(f"Wartung in {wartung_in:,} h".replace(",", "."))
            self._lbl_wartung.setStyleSheet("font-size: 9px; color: #666;")
...
class KryoStatusPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # Header
        header = QHBoxLayout()
        for txt, w in [("", 12), ("Kryo", 52), ("Stunden", 62), ("Wartung", 100), ("", 48)]:
            lbl = QLabel(txt)
            lbl.setFixedWidth(w)
    ...
    def _aktualisieren(self):
        def _run():
            ...
            try:
                ...
                print(f"[KryoStatusPanel] XSP01R: {e}")
