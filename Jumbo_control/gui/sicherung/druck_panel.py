...
class DruckPanel(QWidget):
    ...
    def _build_anzeigen(self) -> QWidget:
        ...
        for name, farbe in SENSOR_FARBEN.items():
            ...
            lbl = QLabel("---")
            lbl.setStyleSheet(
                f"font-family: 'Consolas','Courier New'; font-size: 18px; "
    ...
    def _blink(self):
        ...
        for name in self._alarmierende:
            if name in self._anzeigen:
                ...
                lbl.setStyleSheet(
                    f"font-family: 'Consolas','Courier New'; font-size: 18px; font-weight: bold; color: {col}; background: {bg}; border-radius: 3px;"
                )
