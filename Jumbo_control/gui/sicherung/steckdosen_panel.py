...
V1_DRUCKGRENZE     = 1.0    # mbar – V1 gesperrt wenn Druck UNTER diesem Wert
ROOTS_DRUCKGRENZE  = 40.0   # mbar – Roots gesperrt wenn Druck ÜBER diesem Wert
ROOTS_MODUS        = "any"  # "any" = mind. 1 Sensor unter Grenze, "all" = alle drei
...
class SteckdosenPanel(QWidget):
    ...
    def __init__(self):
        ...
        self._druck_werte = {}
        self._v1_gesperrt     = True   # sicher starten
        self._roots_gesperrt  = True   # sicher starten
        self._heater_gesperrt = False
        self._kryo_an_check   = None   # Callback: True wenn mind. 1 Kryo an
    ...
    def _schalten(self, name: str, btn: QPushButton):
        ...
        if name == "Heater" and btn.isChecked():
            ...
            if self._kryo_an_check and self._kryo_an_check():
                btn.setChecked(False)
                if self.bei_aktion:
                    self.bei_aktion("⛔ Heater kann nicht eingeschaltet werden – Kryos sind aktiv!")
                return
        # Sicherheitscheck Roots
        if name == "Roots" and self._roots_gesperrt:
            btn.setChecked(False)
            if self.bei_aktion:
                self.bei_aktion("⛔ Roots Schaltversuch blockiert – Druck zu hoch!")
            return
        # Sicherheitscheck für V1
    ...
    # ── Heater-Verriegelung (von KryoStatusPanel gesteuert) ──
